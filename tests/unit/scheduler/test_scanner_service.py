...
from tradebot.broker.instruments import InstrumentInfo
...
async def seeded_db(db_engine):
    """БД с 60 свечами SBER (watchlist и instruments теперь из yaml/API)."""
...
class TestScannerServiceRunOnce:
    ...
    async def test_empty_watchlist_skips_scan(self, db_engine):
        ...
        with patch("tradebot.scheduler.scanner_service.load_tickers", return_value=[]):
    ...
    @pytest.mark.asyncio
    async def test_instrument_not_found_in_api_skips_ticker(self, db_engine):
        """Ticker не найден в API → пропускаем, notifier молчит."""
        ...
        with (
            patch("tradebot.scheduler.scanner_service.load_tickers", return_value=["UNKN"]),
            patch(
                "tradebot.scheduler.scanner_service.InstrumentResolver.resolve",
                new=AsyncMock(side_effect=InstrumentNotFoundError("UNKN")),
            ),
        ):
            service = ScannerService(settings, client, notifier)
            await service.run_once()
    ...
    async def test_not_tradeable_saves_no_signal(self, seeded_db):
        ...
        with (
            patch("tradebot.scheduler.scanner_service.load_tickers", return_value=["SBER"]),
            patch(
                "tradebot.scheduler.scanner_service.InstrumentResolver.resolve",
                new=AsyncMock(return_value=_SBER_INFO),
            ),
            patch(
